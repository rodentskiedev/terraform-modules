...
Each entry is one (account, group, permission set) combination. Obtain `instance_arn` from the `data/sso_instance` module, `account_id` from the `data/accounts` module, `group_id` from the `aws_groups` module, and `permission_set_arn` from the `aws_permission_sets` module.
